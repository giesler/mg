...
// first, some types:
typedef unsigned      long uint4; // assumes integer is 4 b long
typedef unsigned short int uint2; // assumes short integer is 2 b long
typedef unsigned      char uint1; // assumes char is 1 b long
...
public:
	...
	//load & save
	bool New();
	bool Load(IXMLDOMDocument* xml, char* version);
	bool LoadFromFile(char* pFile, char* version);
	bool Save(IXMLDOMDocument** xml, char* version);
	bool SaveToFile(char* pFile, char* version);
	bool SaveToDefaultFile(char* version);
