...
static LRESULT CALLBACK BG_WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg)
  {
    ...
    // this portion by Drew Davidson, drewdavidson@mindspring.com
		// ABD: this method of drawing the gradient is *much* faster/more responsive, especially on 32 bpp displays.
		// To test the speedup, switch to a full-screen app, and then back to the installer.  The old method
		// sloooooowly draws the gradient when the window is revealed.  The new method pops up the gradient pretty 
		// much instantly. This drawing method looks largely the same as the above, but we draw in strips 10 
		// pixels high, instead of creating/destroying a pen for every single horizontal line.
        ...
        if (m_textcolor != -1)
        {
          newFont = CreateFont(40,0,0,0,FW_BOLD,TRUE,FALSE,FALSE,ANSI_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH,"Garamond");
          if (newFont)
          {
            static char buf[256];
            r.left+=16;
