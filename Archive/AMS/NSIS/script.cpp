...
int CEXEBuild::process_line(char *str)
{
  if (has_called_write_output)
  {
    printf("Error (process_script): write_output already called, can't continue\n");
...
int CEXEBuild::parseLine(char *str, FILE *fp, char *curfilename, int *lineptr, int ignore)
{
  ...
  if (m_lastline_cat)
  {
    char *t=(char*)malloc(strlen(m_lastline_cat)+strlen(str)+1);
    strcpy(t,m_lastline_cat);
    strcat(t,str);
    free(m_lastline_cat);
    res=line.parse(t);
    if (res>0)
...
int CEXEBuild::parseScript(FILE *fp, char *curfilename, int *lineptr, int ignore)
{
  ...
  for (;;)
  {
    ...
    int ret=parseLine(str,fp,curfilename,lineptr,ignore);
    if (ret != PS_OK) return ret;
  }
  return PS_EOF;
}
    ...
    case TOK_DELETEREGKEY:
      {
        ...
        ent.offsets[1]=add_string(line.gettoken_str(2));
        ent.offsets[2]=(which_token==TOK_DELETEREGKEY)?-1:add_string(line.gettoken_str(3));
