...
int CEXEBuild::add_section(char *secname, char *file, int line)
{
  ...
  build_cursection->default_state=(build_header.silent_install||!n||!secname[0])?0x80000000:0;
...
int CEXEBuild::add_entry(entry *ent)
{
  ...
  if (!uninstall_mode)
  {
    if (!build_cursection_isfunc && build_strlist.get()[build_cursection->name_ptr] == '-')
    {
