...
void CAutorunDlg::PerformButtonAction(CDlgButton* button)
{
	...
	if (button->mblnComponentCheck) 
	{
		...
		if (SysUpdates()) 
		{
			...
			// TODO: Ensure first time
			// If the first time, confirm we can go ahead and install
			CUpdateWarning warning;
			warning.SetComponents(mlstComps);
			if (warning.DoModal() == IDCANCEL)
			{
				gLog.LogEvent("User cancelled updates.");
				ShowWindow(SW_SHOW);		// TODO: maybe we shouldn't do this, depending on how we got here
				return;
			}
...
bool CAutorunDlg::SysUpdates() {
	...
	for (i = 0; i < mlstComps.GetCount(); i++) 
	{
		...
		{
			// we have already tried to install this component
