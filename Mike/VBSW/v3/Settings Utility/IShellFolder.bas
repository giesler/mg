...
' Rtns the reference count for *any* (yep, I said *any*) specified object...
...
' Returns a complex pidl (realtive to the desktop) from a special folder's ID.
...
' Returns an absolute pidl (relative to the desktop) from a file system path only.
'   hWnd - handle of window that will own any displayed msg boxes
'   sPath - fully qualified path
' (i.e. not from a display name)
' (calling proc is responsible for freeing the pidl)
...
Public Function GetPIDLFromPath(hWnd As Long, sPath As String) As Long
  ...
  If SUCCEEDED(isfDesktop.ParseDisplayName(hWnd, 0, _
...
' Returns a relative pidl's displayname
...
' (GetFileDisplayName() only works on absolute pidls)
  ...
  If (SUCCEEDED(isfParent.GetDisplayNameOf(pidlRel, uFlags, lpStr))) Then
    Select Case (lpStr.uType)
      ...
      Case STRRET_OFFSET:
        ...
        sRtn = GetStrFromPtrA(pidlRel + uOffset)
      
      ' The display name is returned in cStr.
      Case STRRET_CSTR:
        sRtn = GetStrFromPtrA(VarPtr(lpStr.CStr(0)))
    
    End Select
  End If
  
  GetFolderDisplayName = sRtn

End Function

' Retrieves a pointer to a relative pidl's ANSII displayname string
' (the relative pidl must be a child of isfParent)
'   isfParent - parent folder's IShellFolder
'   pidlRel    - child object's relative pidl we're getting the name of
'   uFlags    - specifies the type of name to retrieve
'   lpszA      - filled w/ the pointer to the ANSII string buffer,
'                   does not need to be pre-allocated

' Returns True on success, False on failure
' (GetFileDisplayName() only works on absolute pidls)

Public Function GetFolderDisplayNameEx(isfParent As IShellFolder, _
                                                                   pidlRel As Long, _
                                                                   uFlags As ESHGNO, _
                                                                   lpszA As Long) As Boolean
  Static lpStr As STRRET   ' struct filled
  Static lpszW As Long      ' Unicode string pointer
  Static uOffset As Long     ' offset to the string pointer
  ...
  If isfParent.GetDisplayNameOf(pidlRel, uFlags, lpStr) = NOERROR Then
    Select Case (lpStr.uType)
      ...
      ' str which *should* be allocated & freed (?).
      Case STRRET_WSTR:
        MoveMemory lpszW, lpStr.CStr(0), 4
        lpszA = StrPtr(String$(MAX_PATH, 0))
        Call WideCharToMultiByte(CP_ACP, 0, ByVal lpszW, -1, ByVal lpszA, MAX_PATH, 0, 0)
      
      ' The 1st UINT (Long) of the array points to the location
      ' (uOffset bytes) to the ANSII str in the pidl.
      Case STRRET_OFFSET:
        MoveMemory uOffset, lpStr.CStr(0), 4
        lpszA = pidlRel + uOffset
'Debug.Print GetStrFromPtrA(lpszA); uOffset; GetPIDLSize(pidlRel)

      ' The display name is returned in cStr.
      Case STRRET_CSTR:
        lpszA = VarPtr(lpStr.CStr(0))
    
    End Select
    GetFolderDisplayNameEx = True
