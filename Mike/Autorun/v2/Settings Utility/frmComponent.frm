...
Begin VB.Form frmComponent 
   ...
   Begin TabDlg.SSTab tb 
      ...
      Tab(0).Control(6)=   "fraCheckType"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "txtURL"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).ControlCount=   8
      TabCaption(1)   =   "OS Requirements"
      ...
      Begin VB.TextBox txtID 
         ...
         Left            =   1200
         TabIndex        =   1
         ToolTipText     =   "ID to be used internally, no spaces"
         Top             =   1200
         Width           =   1815
      End
...
Me.txtID = mobjComponent.ID
Me.txtName = mobjComponent.Name
Me.Caption = Me.txtID & ": " & Me.txtName

' load other components
Dim objCompList As CComponents, objComp As CComponent, li As ListItem
For Each objComp In mobjComponent.Settings.CComponents
  If objComp.UniqueID <> mobjComponent.UniqueID Then
    Set li = Me.lvDependencies.ListItems.Add(, objComp.UniqueID, objComp.ID)
    li.SubItems(1) = objComp.Name
    If InStr(mobjComponent.Dependencies, " " & li.Text & " ") > 0 Then
      li.Checked = True
    End If
  End If
Next objComp
...
' fix depends if id changed
If mobjComponent.ID <> Me.txtID Then
  For Each objComp In mobjComponent.Settings.CComponents
    objComp.Dependencies = Replace(objComp.Dependencies, " " & mobjComponent.ID & " ", _
          " " & Me.txtID & " ")
  Next objComp
End If

mobjComponent.ID = Me.txtID
mobjComponent.Name = Me.txtName
mobjComponent.URL = Me.txtURL

mobjComponent.Notes = Me.txtNotes
...
If InStr(Me.txtID, " ") > 0 Then
  MsgBox "You cannot have a space in the component ID.", vbExclamation
  Me.tb.Tab = 0
  Me.txtID.SetFocus
  Exit Function
End If
