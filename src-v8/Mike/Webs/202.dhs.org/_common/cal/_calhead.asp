...
<!-- begin calander include -->

<IFRAME STYLE="display:none;position:absolute;width:148;height:194;z-index=100" 
	ID="CalFrame" MARGINHEIGHT=0 MARGINWIDTH=0 NORESIZE FRAMEBORDER=0 SCROLLING=NO 
	SRC="<%=mstrOffset%>_common/cal/calendar.asp"></IFRAME>
...
function DefYr(m,d){
	var dt=new Date();
	var yCur=(dt.getYear()<1000)?1900+dt.getYear():
	dt.getYear();
...
function SetDate(d,m,y) {
	...
	if (g_dFmt=="mmddyy")
		...
		g_eC.value=d+ds+m+ds+y;g_eCV=g_eC.value;
...
function LoadMonths(n) {
	...
	for (i=0;i<n;i++) {
		...
		rg[i].src="<%=mstrOffset%>_common/cal/months/w"+GetDOW2(1,m,y)+"d"+GetMonthCount(m,y)+".gif";
		m++;
		if (12<m){
			m=1;
			y++;
...
function AttB(f) {
	if(bVer.ie4)f.style.display='block';
}

function AttN(f) {
	if(bVer.ie4)f.style.display='none';
}
