...
BOOL CAutorunDlg::OnInitDialog()
{
	...
	util u;
	CString sAppName = u.GetAppTitle();
	CString sSplash  = u.GetBMPFileName();

	SetWindowText(sAppName);

	// check for picture
	if (sSplash.GetLength() > 0 ) {
		sSplash = u.EXEPath() + sSplash;
		HBITMAP hBmp = (HBITMAP)::LoadImage(NULL, sSplash, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		if (hBmp == NULL) {
			u.DisplayError();
		} else {
