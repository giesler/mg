...
'***************************************************************************
'Written and modified by Karl E. Peterson, February 1994, CIS 72302,3707.
'Portions originally downloaded from CompuServe's MSBASIC forum as
'MINIFILE.BAS, author unknown.  Comments and questions welcome! (See below.)
'***************************************************************************
...
'(NOTE: Profile calls *altered* from those found in WIN30API.TXT!)
  ...
  Declare Function kpSendMessage Lib "User" Alias "SendMessage" (ByVal hWnd As Long, ByVal wMsg As Long, ByVal wParam As Long, lParam As Any) As Long
...
Public Function PrivGetSectEntriesEx(sTable() As String) As Integer
...
Private Sub PrivIniNotReg()
  ...
  'Warn *PROGRAMMER* that there's a logic error!
    MsgBox "PrivININotReg", "[Section] and FileName Not Registered in Private.Ini!", 16, "IniFile Logic Error"
...
Public Function InitCDRom() As Boolean
On Error Resume Next

InitCDRom = False

Dim CurDrive As String, CurPath As String

CurPath = App.Path
If Left$(CurPath, 2) = "\\" Then
    MsgBox "The TargetPRO Demo program cannot be run from a network share.  It must be installed directly from the CD-ROM.", vbExclamation
    Exit Function
ElseIf Mid$(CurPath, 2, 1) <> ":" Then
    MsgBox "The TargetPRO Demo program cannot be installed from a network share.  It must be installed directly from the CD-ROM.", vbExclamation
    Exit Function
Else
    CurDrive = Left$(CurPath, 1)
    Exit Function
End If

WinIniRegister ("TargetPRO")
If Not (WinPutString("CD", CurDrive)) Then
    MsgBox "The install program failed to save the CD-ROM information.", vbCritical
    Exit Function
End If
