namespace System.Data.Services.Http
{
    ...
    internal class CaseInsensitiveAscii : IEqualityComparer<string>, IComparer<string>
    {
        ...
        public int Compare(string firstString, string secondString)
        {
            ...
            int num = firstString.Length - secondString.Length;
            int num2 = (num > 0) ? secondString.Length : firstString.Length;
            for (int i = 0; i < num2; i++)
            {
                int num4 = AsciiToLower[firstString[i]] - AsciiToLower[secondString[i]];
                if (num4 != 0)
                {
                    return num4;
                }
        ...
        public bool Equals(string firstString, string secondString)
        {
            ...
            if (secondString != null)
            {
                ...
                if ((length == secondString.Length) && (FastGetHashCode(firstString) == FastGetHashCode(secondString)))
                {
                    int num2 = firstString.Length;
                    while (length > 0)
                    {
                        length--;
                        if (AsciiToLower[firstString[length]] != AsciiToLower[secondString[length]])
                        {
                            return false;
                        }
                    }
        ...
        private static int FastGetHashCode(string text)
        {
            ...
            if (length != 0)
            {
                length ^= (AsciiToLower[(byte) text[0]] << 0x18) ^ (AsciiToLower[(byte) text[length - 1]] << 0x10);
            }
            return length;
        }
        ...
        public int GetHashCode(string text)
        {
            ...
            return (length ^ ((AsciiToLower[(byte) text[0]] << 0x18) ^ (AsciiToLower[(byte) text[length - 1]] << 0x10)));
        }
    }
}
