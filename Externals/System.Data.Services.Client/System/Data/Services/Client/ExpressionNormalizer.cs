namespace System.Data.Services.Client
{
    ...
    internal class ExpressionNormalizer : DataServiceALinqExpressionVisitor
    {
        ...
        private static readonly MethodInfo s_relationalOperatorPlaceholderMethod = typeof(ExpressionNormalizer).GetMethod("RelationalOperatorPlaceholder", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        private static BinaryExpression CreateRelationalOperator(ExpressionType op, Expression left, Expression right)
        {
            ...
            if (!TryCreateRelationalOperator(op, left, right, out expression))
            {
        ...
        private static bool TryMatchCoalescePattern(Expression expression, out Expression normalized)
        {
            ...
            if (expression.NodeType == ExpressionType.Lambda)
            {
                ...
                if ((body.Right.NodeType == ExpressionType.Constant) && 0.Equals(((ConstantExpression) body.Right).Value))
                {
        ...
        internal Expression VisitMethodCallNoRewrite(MethodCallExpression call)
        {
            ...
            return NormalizePredicateArgument(callExpression);
        }
        ...
        internal override Expression VisitUnary(UnaryExpression u)
        {
            ...
            if (((expression.NodeType == ExpressionType.Convert) || (expression.NodeType == ExpressionType.TypeAs)) && expression.Type.IsAssignableFrom(expression.Operand.Type))
            {
                if ((ClientConvert.IsKnownNullableType(expression.Operand.Type) || ClientConvert.IsKnownNullableType(expression.Type)) && !(expression.Operand.Type == expression.Type))
                {
                    return rewritten;
                }
                if (!(ClientType.CheckElementTypeIsEntity(expression.Operand.Type) && ProjectionAnalyzer.IsCollectionProducingExpression(expression.Operand)))
                {
                    rewritten = expression.Operand;
                }
            }
            return rewritten;
        }
