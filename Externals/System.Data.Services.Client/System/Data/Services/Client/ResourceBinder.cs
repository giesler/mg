namespace System.Data.Services.Client
{
    ...
    internal class ResourceBinder : DataServiceALinqExpressionVisitor
    {
        ...
        private static Expression AnalyzePredicate(MethodCallExpression mce)
        {
            ...
            if (list4 != null)
            {
                ...
                if (ExtractKeyPredicate(expression, list4, out dictionary3))
                {
                    ...
                    {
                        expression7 = BuildKeyPredicateFilter(expression.CreateReference(), dictionary3);
                ...
                {
                    expression7 = BuildKeyPredicateFilter(expression.CreateReference(), expression.KeyPredicate);
                    list4.Add(expression7);
        ...
        {
            Debug.Assert(mce != null, "mce != null");
            Debug.Assert((sequenceMethod == SequenceMethod.Select) || (sequenceMethod == SequenceMethod.SelectManyResultSelector), "sequenceMethod == SequenceMethod.Select(ManyResultSelector)");
            e = mce;
            ...
            if (sequenceMethod == SequenceMethod.SelectManyResultSelector)
            {
                ...
                expression4 = ExpressionHelpers.CreateLambda(expression4.Body, new ParameterExpression[] { expression4.Parameters[1] });
        ...
        private static Expression AnalyzeResourceSetConstantMethod(MethodCallExpression mce, Func<MethodCallExpression, ResourceExpression, ConstantExpression, Expression> constantMethodAnalyzer)
        {
            ...
            return constantMethodAnalyzer.Invoke(mce, expression, expression2);
        }
        ...
        private static Expression AnalyzeResourceSetMethod(MethodCallExpression mce, Func<MethodCallExpression, ResourceSetExpression, Expression, Expression> sequenceMethodAnalyzer)
        {
            ...
            return sequenceMethodAnalyzer.Invoke(mce, expression, expression3);
        }
        ...
        private static Expression AnalyzeSelectManySelector(MethodCallExpression selectManyCall, ResourceSetExpression sourceResourceSet)
        {
            ...
            {
                if (!ProjectionAnalyzer.Analyze(ExpressionHelpers.CreateLambda(resultSelector.Body, new ParameterExpression[] { resultSelector.Parameters[1] }), sourceResourceSet, false))
                {
        ...
        private static Expression BuildKeyPredicateFilter(InputReferenceExpression input, Dictionary<PropertyInfo, ConstantExpression> keyValuesDictionary)
        {
            ...
            {
                Expression right = Expression.Equal(Expression.Property(input, pair.Key), pair.Value);
                if (left == null)
                {
                    left = right;
                }
        ...
        private static bool CollectionContentsEqual<T>(ICollection<T> left, ICollection<T> right, IEqualityComparer<T> comparer) where T: class
        {
            ...
            foreach (T local in left)
            {
                set.Add(local);
            }
            foreach (T local2 in right)
            {
        ...
        {
              NavigationPropertySingletonExpression expression = new NavigationPropertySingletonExpression(type, source, memberExpression, memberExpression.Type, source.ExpandPaths.ToList<string>(), source.CountOption, source.CustomQueryOptions.ToDictionary<KeyValuePair<ConstantExpression, ConstantExpression>, ConstantExpression, ConstantExpression>(delegate (KeyValuePair<ConstantExpression, ConstantExpression> kvp) {
        return kvp.Key;
    ...
    return expression;
        ...
        private static bool ExtractKeyPredicate(ResourceSetExpression target, List<Expression> predicates, out Dictionary<PropertyInfo, ConstantExpression> keyValues)
        {
            ...
            Debug.Assert((keyValues != null) || (collection != null), "No key predicates or non-key predicates found?");
        ...
        private static ResourceSetExpression RemoveTransparentScope(Type expectedResultType, ResourceSetExpression input)
        {
            ...
            expression.KeyPredicate = input.KeyPredicate;
        ...
        internal static void VerifyNotSelectManyProjection(Expression expression)
        {
            ...
            if (expression2 != null)
            {
                ...
                if (projection != null)
                {
                    ...
                    {
                        throw new NotSupportedException(Strings.ALinq_UnsupportedExpression(expression4));
                    }
                }
                else if (expression2.HasTransparentScope)
                {
                    throw new NotSupportedException(Strings.ALinq_UnsupportedExpression(expression2));
                }
            }
        }
        ...
        internal override Expression VisitMethodCall(MethodCallExpression mce)
        {
            ...
            if (!ReflectionUtil.TryIdentifySequenceMethod(mce.Method, out method))
            {
                ...
                if (object.Equals(mce.Method, type.GetMethod("Expand", new Type[] { typeof(string) })))
                {
                    return AnalyzeExpand(mce);
                }
                if (object.Equals(mce.Method, type.GetMethod("AddQueryOption", new Type[] { typeof(string), typeof(object) })))
                {
                    return AnalyzeAddCustomQueryOption(mce);
                }
                if (!object.Equals(mce.Method, type.GetMethod("IncludeTotalCount")))
                {
            ...
            switch (method)
            {
                ...
                case SequenceMethod.OrderBy:
                    return AnalyzeResourceSetMethod(mce, delegate(MethodCallExpression callExp, ResourceSetExpression resource, Expression selector)
                    {
                        return ApplyOrdering(callExp, resource, selector, false, false);
                    });

                case SequenceMethod.OrderByDescending:
                    return AnalyzeResourceSetMethod(mce, delegate(MethodCallExpression callExp, ResourceSetExpression resource, Expression selector)
                    {
                        return ApplyOrdering(callExp, resource, selector, true, false);
                    });

                case SequenceMethod.ThenBy:
                    return AnalyzeResourceSetMethod(mce, delegate(MethodCallExpression callExp, ResourceSetExpression resource, Expression selector)
                    {
                        return ApplyOrdering(callExp, resource, selector, false, true);
                    });

                case SequenceMethod.ThenByDescending:
                    return AnalyzeResourceSetMethod(mce, delegate(MethodCallExpression callExp, ResourceSetExpression resource, Expression selector)
                    {
                        return ApplyOrdering(callExp, resource, selector, true, true);
                    });
        ...
        internal static class PatternRules
        {
            ...
            internal static bool MatchAnd(Expression e)
            {
                ...
                return ((expression != null) && ((expression.NodeType == ExpressionType.And) || (expression.NodeType == ExpressionType.AndAlso)));
            }
            ...
            internal static MatchNullCheckResult MatchNullCheck(Expression entityInScope, ConditionalExpression conditional)
            {
                ...
                if (result2.Match)
                {
                    ...
                    Debug.Assert(ifFalse != null, "assignedCandidate != null");
                    Debug.Assert(testRight != null, "memberCandidate != null");
            ...
            internal static bool MatchTransparentIdentitySelector(Expression input, LambdaExpression selector)
            {
                ...
                Debug.Assert(list != null, "refPath != null -- otherwise MatchPropertyAccess should not have returned true");
