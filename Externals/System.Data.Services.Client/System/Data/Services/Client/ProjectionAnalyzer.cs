namespace System.Data.Services.Client
{
    ...
    internal static class ProjectionAnalyzer
    {
        ...
        private class EntityProjectionAnalyzer : ALinqExpressionVisitor
        {
            ...
            internal static void Analyze(MemberInitExpression mie, PathBox pb)
            {
                ...
                foreach (MemberBinding binding in mie.Bindings)
                {
                    ...
                    if (assignment != null)
                    {
                        ...
                        {
                            throw new NotSupportedException(Strings.ALinq_ExpressionNotSupportedInProjectionToEntity(memberType, assignment.Expression));
                        }
                        MemberExpression expression = expressionsBeyondTargetEntity[expressionsBeyondTargetEntity.Length - 1] as MemberExpression;
                        Debug.Assert(!analysis2.MultiplePathsFound, "!analysis.MultiplePathsFound -- the initilizer has been visited, and cannot be empty, and expressions that can combine paths should have thrown exception during initializer analysis");
                        Debug.Assert(expression != null, "lastExpression != null -- the initilizer has been visited, and cannot be empty, and the only expressions that are allowed can be formed off the parameter, so this is always correlatd");
                        if ((expression != null) && (expression.Member.Name != assignment.Member.Name))
                        {
                            throw new NotSupportedException(Strings.ALinq_PropertyNamesMustMatchInProjections(expression.Member.Name, assignment.Member.Name));
                        }
                        ...
                        {
                            throw new NotSupportedException(Strings.ALinq_ExpressionNotSupportedInProjection(memberType, assignment.Expression));
                        }
                    }
                }
            }
            ...
            private static Type GetMemberType(MemberInfo member)
            {
                ...
                Debug.Assert(info2 != null, "fieldInfo != null -- otherwise Expression.Member factory should have thrown an argument exception");
            ...
            {
                if (((m.Object != null) && ProjectionAnalyzer.IsDisallowedExpressionForMethodCall(m.Object)) || m.Arguments.Any<Expression>(delegate (Expression a) {
        return ProjectionAnalyzer.IsDisallowedExpressionForMethodCall(a);
    }))
    {
    ...
    return base.VisitMethodCall(m);
        ...
        return (ClientType.CheckElementTypeIsEntity(a.Type) || ProjectionAnalyzer.IsCollectionProducingExpression(a));
    }))
    {
    ...
    return base.VisitInvocation(iv);
            ...
            internal override Expression VisitMethodCall(MethodCallExpression m)
            {
                ...
                if (((m.Object != null) ? ClientType.CheckElementTypeIsEntity(m.Object.Type) : false) || m.Arguments.Any<Expression>(delegate(Expression a)
                {
                    return ClientType.CheckElementTypeIsEntity(a.Type);
                }))
                {
                    throw new NotSupportedException(Strings.ALinq_ExpressionNotSupportedInProjection(this.type, m.ToString()));
                }
