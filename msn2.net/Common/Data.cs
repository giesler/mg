	...
	public class Data //: System.Windows.Forms.TreeNode
	{
		...
		internal Data(DataSetDataItem.DataItemRow row, Type type, string storageUrl)
		{
			...
			if (type != null && !row.IsItemTypeNull() && row.ItemType.Length > 0)
			{
				...
				{
					this.configData = (ConfigData) msn2.net.Common.Utilities.DeserializeObject(row.ItemData, this.dataType);
				}
			}
...
//				dataService.GetChildren(this.id, this.userId, typeList);
		...
		public Data Get(string name, ConfigData data, ConfigTreeLocation configTreeLocation)
		{
			return Get(name, null, data, null, configTreeLocation);
		}

		public Data Get(string name, ConfigData data, Type type)
		{
			return Get(name, null, data, type);
		}

		public Data Get(string name, ConfigData data, Type type, ConfigTreeLocation configTreeLocation)
		{
		...
		public Data Get(string name, string url, ConfigData data)
		{
			return Get(name, url, data, data.GetType());
		}

		public Data Get(string name, string url, ConfigData data, ConfigTreeLocation configTreeLocation)
		{
			return Get(name, url, data, data.GetType(), configTreeLocation);
		}
	

		public Data Get(string name, string url, ConfigData data, Type type)
		{
		...
		public Data Get(string name, string url, ConfigData data, Type type, ConfigTreeLocation configTreeLocation)
		{
...
//			dataService.Delete(this.id);
		...
		private Guid LookupTheirGroupId(object contactData)
		{
